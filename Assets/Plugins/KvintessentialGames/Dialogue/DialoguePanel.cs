  ...
  public class DialoguePanel : MonoBehaviour
    {
        ...
        public event Action OnDialogueShow; 
        public event Action OnDialogueClear; 

        public void InitAudioService(AudioService audioService)
        {
        ...
        private async UniTask SafeShowSequence(Dialogue sequence, Action onComplete = null, CancellationToken token = default)
        {
            try
            {
                await ShowSequence(sequence, onComplete, token);
            }
            catch (Exception e)
            {
                Debug.LogError(e);
        ...
        public async UniTask ShowDialogueAsync(Dialogue sequence, CancellationToken token = default)
        {
            await UniTask.WaitWhile(() => isShowing, cancellationToken: token);
            await SafeShowSequence(sequence, token: token);
        }
        ...
        private async UniTask ShowSequence(Dialogue sequence, Action onComplete = null, CancellationToken token = default)
        {
            ...
            for (var i = 0; i < sequence.Stages.Count; i++)
            {
                ...
                if (stage.Type == DialogueStageType.Text)
                {
                    ...
                    for (var j = 0; j < stage.Text.Length && characterIndex < text.textInfo.characterInfo.Length; j++, characterIndex++)
                    {
                        if (token.IsCancellationRequested)
                        {
                            break;
                        ...
                        if (stage.WithSpeaker && nextTimeSound <= Time.time && char.IsLetter(c))
                        {
                            ...
                            var sound = audioService.PlaySound(
                                $"Typing_{speaker}", 0.1f, 0.5f, pitch: isSkipping ? 1.5f : 1f);
                                
                            var length = sound != null ? sound.Clip.length : 0.1f;
                        ...
                        if (delay != 0)
                        {
                            ...
                            await UniTask.Delay((int)(delay * multiplier), cancellationToken: token);
                        }
                    }
                }
                ...
                if (stage.Type == DialogueStageType.Clear)
                {
                    ...
                    if (previousText.Length != 0)
                        await UniTask.WaitWhile(() => text.textInfo.characterCount == 0, cancellationToken: token);
                ...
                if (stage.Type == DialogueStageType.Animation)
                {
                    ...
                    await UniTask.WaitWhile(() => fallLettersAnimation.IsPlaying, cancellationToken: token);
                }
            }
